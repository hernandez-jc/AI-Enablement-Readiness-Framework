...
Una forma sencilla de establecer esta relación es:
...
Esto permite evitar una situación común:

"Tenemos una herramienta y estamos buscando dónde utilizarla."

El enfoque estratégico invierte la lógica:
...
| ------ | --------------------------------- | ------------------------------------------------------------------- |
| STR-01 | Prioridades estratégicas          | Si las iniciativas de IA responden a prioridades organizacionales   |
| STR-02 | Objetivos de negocio              | Si cada iniciativa tiene resultados empresariales definidos         |
| STR-03 | Identificación de oportunidades   | Si existe capacidad para identificar oportunidades relevantes       |
| STR-04 | Priorización de casos de uso      | Si existen criterios para seleccionar iniciativas                   |
| STR-05 | Valor esperado                    | Si puede definirse el beneficio esperado de una iniciativa          |
| STR-06 | Recursos y capacidad de ejecución | Si existen recursos suficientes para implementar las prioridades    |
| STR-07 | Patrocinio ejecutivo              | Si existe apoyo y responsabilidad a nivel de liderazgo              |
| STR-08 | Alineación entre áreas            | Si existe coordinación entre equipos y funciones                    |
...
Una iniciativa puede ser técnicamente interesante y no ser estratégicamente prioritaria.

La alineación permite concentrar recursos donde existe mayor potencial de impacto.

Conexión con el assessment

Una puntuación baja puede indicar que las iniciativas de IA se desarrollan de forma oportunista o aislada.
...
| ------------------------ | --------------------------------------- |
| Reducir tiempo operativo | Menor tiempo por transacción            |
| Mejorar atención         | Menor tiempo de respuesta               |
| Mejorar calidad          | Menor tasa de errores                   |
...
Los colaboradores necesitan aprender a observar su trabajo y reconocer oportunidades de aplicación de IA.
...
Una matriz de priorización puede considerar:

| Criterio      | Pregunta                                           |
| ------------- | -------------------------------------------------- |
| Valor         | ¿Qué beneficio puede generar?                      |
| Factibilidad  | ¿Puede implementarse con las capacidades actuales? |
| Datos         | ¿Existen datos adecuados?                          |
| Riesgo        | ¿Qué consecuencias tendría un error?               |
| Esfuerzo      | ¿Qué recursos requiere?                            |
| Tiempo        | ¿Cuánto tardaría en producir resultados?           |
| Escalabilidad | ¿Puede extenderse a otros procesos?                |
| Alineación    | ¿Qué tan directamente contribuye a una prioridad?  |

Esto permite comparar iniciativas con criterios comunes.
...
la diferencia que produce en el resultado que importa.

Ejemplo

Una iniciativa puede generar miles de respuestas automáticamente.

Eso no demuestra por sí mismo que haya generado valor.
...
Una organización puede identificar excelentes oportunidades y aun así no tener capacidad para ejecutarlas.

Conexión con Skills

Aquí aparece una conexión importante:

La estrategia define qué se quiere lograr; las capacidades determinan qué tan viable es lograrlo.
...
El patrocinio efectivo implica más que aprobar un proyecto.
...
La alineación interfuncional permite integrar:

Estrategia + Datos + Skills + Tecnología + Gobernanza + Procesos
...
Puede existir una brecha entre:

Probar → Implementar → Escalar

Conexión con AI Enablement

La capacitación debe considerar no solamente a los primeros usuarios.
...
Los indicadores deben conectarse con el objetivo del caso de uso.

| Objetivo               | Indicador posible                  |
| ---------------------- | ---------------------------------- |
| Reducir tiempo         | Tiempo promedio de ejecución       |
| Mejorar calidad        | Tasa de errores                    |
| Aumentar productividad | Producción por unidad de tiempo    |
| Mejorar servicio       | Tiempo de respuesta o satisfacción |
| Reducir riesgo         | Incidentes o pérdidas evitadas     |
| Mejorar cumplimiento   | Tasa de cumplimiento               |
| Mejorar experiencia    | Indicadores de satisfacción        |
...
| Nivel     | Ejemplo                                        |
| --------- | ---------------------------------------------- |
| Actividad | Se implementó una herramienta                  |
| Adopción  | Los empleados comenzaron a utilizarla          |
| Uso       | La herramienta se utiliza regularmente         |
| Desempeño | La tarea se realiza mejor o más rápido         |
...
| ----: | ------------- | -------------------------------------------------------------------------------- |
|     1 | Inicial       | La adopción ocurre principalmente de forma experimental o aislada                |
|     2 | En desarrollo | Existen iniciativas y prioridades parciales                                      |
|     3 | Alineada      | Las principales iniciativas tienen objetivos y prioridades definidos             |
|     4 | Gestionada    | La cartera de iniciativas se prioriza, gestiona y mide                           |
|     5 | Integrada     | IA está integrada en la estrategia y existe mejora continua basada en resultados |

El score no debe interpretarse como una evaluación absoluta de la organización.
...
| Nivel        | Posible necesidad                       |
| ------------ | --------------------------------------- |
| Usuario      | Identificar oportunidades en su trabajo |
| Especialista | Diseñar casos de uso                    |
| Líder        | Priorizar iniciativas                   |
| Director     | Evaluar portafolios y resultados        |
| Ejecutivo    | Tomar decisiones estratégicas sobre IA  |

Por lo tanto, no existe una única "capacitación estratégica en IA".
...
"Los líderes necesitan capacitación en estrategia de IA."

Es preferible:

"Los líderes pueden evaluar y priorizar casos de uso de IA utilizando criterios de valor, factibilidad, riesgo y alineación estratégica."

A partir de esta capacidad pueden diseñarse actividades específicas.

| Capacidad                 | Actividad                | Evidencia                |
| ------------------------- | ------------------------ | ------------------------ |
| Identificar oportunidades | Workshop de procesos     | Mapa de oportunidades    |
| Formular casos de uso     | Laboratorio              | Caso de uso documentado  |
| Priorizar                 | Simulación de portafolio | Priorización justificada |
| Evaluar valor             | Caso empresarial         | Estimación de impacto    |
| Evaluar riesgos           | Escenario de decisión    | Riesgos identificados    |
...
| ----------- | ----: | -----------: | -----: | ---------------: | --------- |
| Caso A      |  Alto |         Alta |   Bajo |             Alta | 1         |
| Caso B      |  Alto |        Media |  Medio |            Media | 2         |
| Caso C      | Medio |         Alta |   Bajo |             Baja | 3         |
| Caso D      |  Alto |         Baja |   Alto |             Baja | Evaluar   |
...
Objetivo estratégico: mejorar eficiencia operacional
↓
Oportunidad: automatizar tareas administrativas
↓
Caso de uso: automatización asistida por IA
↓
Datos: fuentes disponibles y confiables
↓
Skills: usuarios capaces de supervisar resultados
↓
Gobernanza: reglas de uso y controles
↓
Tecnología: herramienta integrada
↓
Resultado: reducción del tiempo de procesamiento
...
| Elemento           | Pregunta                                     |
| ------------------ | -------------------------------------------- |
| Prioridad          | ¿Qué objetivo estratégico requiere atención? |
| Brecha             | ¿Qué capacidad está limitando el avance?     |
| Causa              | ¿Por qué existe la brecha?                   |
| Acción             | ¿Qué debe cambiar?                           |
| Responsable        | ¿Quién debe actuar?                          |
| Recursos           | ¿Qué se necesita?                            |
| Indicador          | ¿Cómo se medirá el progreso?                 |
...
| ---------- | ------------------------------------------------------ |
| Datos      | ¿Tenemos lo necesario para ejecutar el caso de uso?    |
| Skills     | ¿Tenemos las capacidades para utilizarlo?              |
| Gobernanza | ¿Podemos hacerlo de manera responsable?                |
| Tecnología | ¿Tenemos la infraestructura y herramientas necesarias? |
| Procesos   | ¿Podemos integrarlo en el trabajo?                     |
| Cultura    | ¿La organización está preparada para adoptarlo?        |
| Medición   | ¿Podemos demostrar el valor generado?                  |
...
Una organización no necesita desarrollar todas las capacidades de IA al mismo nivel ni implementar todos los casos de uso posibles.

Necesita identificar:

Qué problemas importan → qué oportunidades existen → qué casos de uso tienen prioridad → qué capacidades se necesitan → qué intervenciones permitirán ejecutarlos → cómo se medirá el resultado.

Por eso, el assessment estratégico debe convertirse en un puente entre:

Estrategia empresarial → Portafolio de IA → Capacidades → Aprendizaje → Implementación → Resultados

La pregunta final no es:

>"¿Qué tan avanzada está nuestra organización en IA?"

Sino:

>"¿Tenemos las prioridades, capacidades y condiciones necesarias para utilizar IA donde pueda generar valor estratégico demostrable?"
